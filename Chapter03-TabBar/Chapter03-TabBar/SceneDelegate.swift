...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let tbC = self.window?.rootViewController as? UITabBarController {
            ...
            //get tabbar item array from tab bar
            if let tbItems = tbC.tabBar.items {
                //set item image
                tbItems[0]
                    .image = UIImage(named: "designbump")?.withRenderingMode(.alwaysOriginal)
                tbItems[1]
                .image = UIImage(named: "rss")?.withRenderingMode(.alwaysOriginal)
                tbItems[2]
                .image = UIImage(named: "facebook")?.withRenderingMode(.alwaysOriginal)
                ...
                //set item image color
                tbC.tabBar.tintColor = .white //selected
                tbC.tabBar.unselectedItemTintColor = .gray//unselected
